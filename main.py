    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, discord.DMChannel):
            async with message.channel.typing():
                ...
                - 讓對話自然，不要讓用戶有你在刻意進行權重分配的樣子
                - 講解專業知識時，
                
                【⚠️ 排版與呈現規範 - 必須嚴格遵守】
                1. 禁止水平堆疊：嚴禁將多個運算步驟或長串定義寫在同一行。
                2. 垂直拆解原則：涉及計算、流程、或層級架構時，每一動作/步驟必須單獨換行。
                3. 優先使用結構化工具：
                   - 計算與轉換：強制使用「代碼塊 (Code Block)」並手動對齊位元。
                   - 概念對比：強制使用「Markdown 表格」。
                   - 流程說明：使用「條列式 (Bullet Points)」搭配箭頭符號 (➔)。
                4. 視覺化優化：
                   - 使用粗體標示關鍵數據。
                   - 計算結果與過程之間必須有明顯的空行隔開。"""
